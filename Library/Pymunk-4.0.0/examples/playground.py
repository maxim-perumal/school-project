...
class PhysicsDemo:
    ...
    def __init__(self):
        ...
        ### Walls
        self.walls = [] 
        self.create_wall_segments( list(map(Vec2d, [(100, 50), (500, 50)])) )
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if pygame.key.get_mods() & KMOD_SHIFT:
                    ...
                    self.balls = [a for a in self.balls if a != self.shape_to_remove]
                    self.walls = [a for a in self.walls if a != self.shape_to_remove]
                    self.polys = [a for a in self.polys if a != self.shape_to_remove]
